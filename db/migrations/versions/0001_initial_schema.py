...
def upgrade() -> None:
    ...
    project_type_enum = postgresql.ENUM(
        "client", "personal", "family", "ops", "writing", "internal",
        name="project_type_enum", create_type=True
    )
    task_kind_enum = postgresql.ENUM(
        "task", "waiting", "commitment", "idea", "reference",
        name="task_kind_enum", create_type=True
    )
    task_status_enum = postgresql.ENUM(
        "new", "proposed", "confirmed", "routed", "completed", "discarded", "error",
        name="task_status_enum", create_type=True
    )
    confidence_band_enum = postgresql.ENUM(
        "low", "medium", "high",
        name="confidence_band_enum", create_type=True
    )
    review_action_enum = postgresql.ENUM(
        "confirm", "edit", "change_project", "change_type", "discard", "show_steps",
        name="review_action_enum", create_type=True
    )

    project_type_enum.create(op.get_bind(), checkfirst=True)
    task_kind_enum.create(op.get_bind(), checkfirst=True)
    task_status_enum.create(op.get_bind(), checkfirst=True)
    confidence_band_enum.create(op.get_bind(), checkfirst=True)
    ...
    op.create_table(
        ...
        sa.Column("project_type", sa.Enum("client", "personal", "family", "ops", "writing", "internal", name="project_type_enum", create_constraint=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("kind", sa.Enum("task", "waiting", "commitment", "idea", "reference", name="task_kind_enum", create_constraint=False), nullable=True),
        sa.Column("status", sa.Enum("new", "proposed", "confirmed", "routed", "completed", "discarded", "error", name="task_status_enum", create_constraint=False), nullable=False, server_default="new"),
        sa.Column("project_id", postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column("next_action", sa.String(500), nullable=True),
        sa.Column("due_hint", sa.String(100), nullable=True),
        sa.Column("confidence_score", sa.Float(), nullable=True),
        sa.Column("confidence_band", sa.Enum("low", "medium", "high", name="confidence_band_enum", create_constraint=False), nullable=True),
    ...
    op.create_table(
        "task_revisions",
        sa.Column("id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("task_item_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("revision_no", sa.Integer(), nullable=False),
        sa.Column("raw_text", sa.Text(), nullable=False),
        sa.Column("proposal_json", postgresql.JSONB(), nullable=False, server_default="{}"),
        sa.Column("user_decision", sa.Enum("confirm", "edit", "change_project", "change_type", "discard", "show_steps", name="review_action_enum", create_constraint=False), nullable=True),
        sa.Column("user_notes", sa.Text(), nullable=True),
        sa.Column("final_title", sa.String(500), nullable=True),
        sa.Column("final_kind", sa.Enum("task", "waiting", "commitment", "idea", "reference", name="task_kind_enum", create_constraint=False), nullable=True),
        sa.Column("final_project_id", postgresql.UUID(as_uuid=True), nullable=True),
